...
class SttResult {
  ...
  const SttResult({required this.supported, this.text = '', this.error});
}
...
class SpeechService {
  ...
  /// Azure STT로 한 번 인식한다. 웹에서만 지원되며, 그 외 환경에서는 supported=false를 반환한다.
  Future<SttResult> recognizeSpeech({String? localeId}) async {
    if (!kIsWeb || !WebSttHelper.isSupported || !TtsService.isAzureEnabled) {
      return const SttResult(supported: false);
    }
    ...
    try {
      final raw = await WebSttHelper.recognizeSpeech(
        TtsService.azureKey,
      ...
      debugPrint('Azure STT 오류: $e');
      return SttResult(supported: true, error: e.toString());
    }
  }
